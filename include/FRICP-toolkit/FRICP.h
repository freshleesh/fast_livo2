...
#include <limits>
#include "Types.h"  // 确保包含当前文件定义的 Vector3
#include "ICP.h"
#include "median.h"
...
class FRICP {
 public:
  ...
  typedef nanoflann::KDTreeAdaptor<MatrixNX, N, nanoflann::metric_L2_Simple>
      KDtree;
 ...
 private:
  ...
  inline AffineMatrixN EulerToRot(const Vector6& v) {
    MatrixNN s(Eigen::AngleAxis<Scalar>(v(0), Types::Vector3::UnitX()) *
               Eigen::AngleAxis<Scalar>(v(1), Types::Vector3::UnitY()) *
               Eigen::AngleAxis<Scalar>(v(2), Types::Vector3::UnitZ()));
  ...
  inline AffineMatrixN VecToLog(const Vector6& v) {
    AffineMatrixN m = AffineMatrixN::Zero();
    m << 0, -v[2], v[1], v[3], v[2], 0, -v[0], v[4], -v[1], v[0], 0, v[5], 0, 0,
        0, 0;
    return m;
  }

  double FindKnearestMed(const KDtree& kdtree, const MatrixNX& X, int nk) {
...
#pragma omp parallel for
  ...
  double FindKnearestNormMed(const KDtree& kdtree, const Eigen::Matrix3Xd& X,
                             int nk, const Eigen::Matrix3Xd& norm_x) {
    Eigen::VectorXd X_nearest(X.cols());
#pragma omp parallel for
    for (int i = 0; i < X.cols(); i++) {
      int* id = new int[nk];
      ...
      for (int s = 1; s < nk; s++) {
        k_dist[s] =
            std::abs((X.col(id[s]) - X.col(id[0])).dot(norm_x.col(id[0])));
      }
      igl::median(k_dist.tail(nk - 1), X_nearest[i]);
      delete[] id;
      delete[] dist;
  ...
  AffineNd point_to_point(Eigen::MatrixBase<Derived1>& X,
    ...
    Eigen::JacobiSVD<MatrixXX> svd(sigma,
...
#pragma omp section
        for (int i = 0; i < C.cols(); i++) {
          double dist_to_plane =
              -((X.col(i) - Y.col(i)).dot(Norm.col(i)) - u(i)) * w(i);
  ...
  double point_to_plane_gaussnewton(const Eigen::MatrixBase<Derived1>& X,
    ...
    Types::Vector3 t = Tk.block(0, 3, 3, 1);
    Types::Vector3 u = log_T.tail(3);
    ...
    if (theta > SAME_THRESHOLD) {
      coeff << st / theta, (1 - ct) / (theta * theta),
          (theta * ct - st) / (theta * theta * theta),
          (theta * st - 2 * (1 - ct)) / pow(theta, 4),
          (1 - ct) / (theta * theta), (theta - st) / pow(theta, 3),
          (theta * st - 2 * (1 - ct)) / pow(theta, 4),
          (theta * (1 - ct) - 3 * (theta - st)) / pow(theta, 5);
    } else
      coeff(0, 0) = 1;
...
#pragma omp for
    for (int i = 0; i < X.cols(); i++) {
      Types::Vector3 xi = X.col(i);
      Types::Vector3 yi = Y.col(i);
      Types::Vector3 ni = norm_y.col(i);
      double wi = sqrt(w_normalized[i]);

      Matrix33 dedR = wi * ni * xi.transpose();
      Types::Vector3 dedt = wi * ni;
 ...
 public:
  void point_to_point(MatrixNX& X, MatrixNX& Y, VectorN& source_mean,
    ...
    Types::VectorX W = Types::VectorX::Zero(X.cols());
    AffineNd T;
    if (par.use_init)
...
#pragma omp parallel for
    ...
    // AA init
    accelerator_.init(par.anderson_m, (N + 1) * (N + 1),
                      LogMatrix(T.matrix()).data());
...
#pragma omp parallel for
        ...
        if (par.use_AA) {
          AffineMatrixN Trans =
              (Eigen::Map<const AffineMatrixN>(
                   accelerator_.compute(LogMatrix(T.matrix()).data()).data(),
                   N + 1, N + 1))
                  .exp();
...
#pragma omp parallel for
    ...
    if (par.print_output) {
      ...
      for (int i = 0; i < times.size(); i++) {
        out_res << times[i] << " " << energys[i] << " " << gt_mses[i]
                << std::endl;
      }
      out_res.close();
  ...
  /// @param Parameters
  //    template <typename Derived1, typename Derived2, typename Derived3>
  void point_to_plane(Eigen::Matrix3Xd& X, Eigen::Matrix3Xd& Y,
                      Eigen::Matrix3Xd& norm_x, Eigen::Matrix3Xd& norm_y,
                      Eigen::Vector3d& source_mean,
                      Eigen::Vector3d& target_mean, ICP::Parameters& par) {
    /// Build kd-tree
    KDtree kdtree(Y);
    /// Buffers
    ...
    AndersonAcceleration accelerator_;
    AffineNd LG_T = T;
    double energy = 0.0, prev_res = std::numeric_limits<double>::max(),
           res = 0.0;
...
#pragma omp parallel for
  ...
  /// @param Parameters
  //    template <typename Derived1, typename Derived2, typename Derived3>
  void point_to_plane_GN(Eigen::Matrix3Xd& X, Eigen::Matrix3Xd& Y,
                         Eigen::Matrix3Xd& norm_x, Eigen::Matrix3Xd& norm_y,
                         Eigen::Vector3d& source_mean,
                         Eigen::Vector3d& target_mean, ICP::Parameters& par) {
    /// Build kd-tree
