...
Eigen::Affine3d point_to_point(Eigen::MatrixBase<Derived1>& X,
  ...
  if (svd.matrixU().determinant() * svd.matrixV().determinant() < 0.0) {
    Types::VectorX S = Types::VectorX::Ones(dim);
    S(dim - 1) = -1.0;
...
#pragma omp parallel for
    ...
    //            if(par.print_icpn)
    //                std::cout << "iter = " << icp << ", time = " << run_time
    //                << ", mse = " << mse << ", gt_mse = " << gt_mse <<
    //                std::endl;
  ...
  if (par.has_groundtruth) gt_mse = (X - X_gt).squaredNorm() / nPoints;
...
void point_to_plane(Eigen::MatrixBase<Derived1>& X,
                    Eigen::MatrixBase<Derived2>& Y,
                    Eigen::MatrixBase<Derived3>& N, Eigen::Vector3d source_mean,
                    Eigen::Vector3d target_mean, Parameters& par) {
  /// Build kd-tree
  nanoflann::KDTreeAdaptor<Eigen::MatrixBase<Derived2>, 3,
                           nanoflann::metric_L2_Simple>
      kdtree(Y);
...
#pragma omp parallel for
    ...
    for (int outer = 0; outer < par.max_outer; ++outer) {
      ...
      for (int inner = 0; inner < par.max_inner; ++inner) {
        ...
        T = RigidMotionEstimator::point_to_plane(
...
int alphas_cond(VectorX alphas) {
  ...
  return alpha_limit_min_ < alphas.minCoeff() &&
         alphas.maxCoeff() < alpha_limit_max_ && alphas(alphas.size() - 1) > 0;
}
...
///////////////////////////////////////////////////////////////////////////////////////////
VectorX get_next_u(const Matrix6X u, const Matrix6X g, const Matrix6X f,
                   std::vector<double>& save_alphas) {
  int i = 1;
  double beta_ = 1.0;
  save_alphas.clear();
  Vector6 sol =
      ((1 - beta_) * u.col(u.cols() - 1) + beta_ * g.col(g.cols() - 1));
  ...
  for (; i <= f.cols(); i++) {
    ...
    sol =
        (1 - beta_) * u.rightCols(i) * alphas + beta_ * g.rightCols(i) * alphas;
    sol_alphas = alphas;
  }
...
template <typename Derived1, typename Derived2, typename Derived3>
void point_to_point_aaicp(Eigen::MatrixBase<Derived1>& X,
                          Eigen::MatrixBase<Derived2>& Y,
                          Eigen::MatrixBase<Derived3>& source_mean,
...
#pragma omp parallel for
    ...
    if (icp) {
      ...
      /// The first heuristic
      if ((energy - prev_energy) / prev_energy >
          par.error_overflow_threshold_) {
    ...
    if (par.print_energy)
      std::cout << "icp iter = " << icp << ", Energy = " << energy
                << ", gt_mse = " << gt_mse << std::endl;
  ...
  final_transformation.block(0, 3, 3, 1) +=
      -final_transformation.block(0, 0, 3, 3) * source_mean + target_mean;
  X.colwise() += target_mean;
