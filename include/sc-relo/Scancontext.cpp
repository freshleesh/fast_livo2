...
std::pair<double, int> SCManager::distanceBtnScanContext(MatrixXd &_sc1,
  ...
  // 上面用sector
  // key匹配，找到一个初始的偏移量，但肯定不是准确的，再在这个偏移量左右扩展一下搜索空间
  // 注意这个SEARCH_RADIUS是区间的一半，即左右偏移。这里是0.5* 10% * 60 =
  // 3，也就是左右扩展3列
 ...
 *
 * @param[in] _scan_down, PointTypeXYZI类型，是pcl::PointXYZI
 * @return MatrixXd, 生成的Scan-Context矩阵
 */
MatrixXd SCManager::makeScancontext(
  ...
  // main
  const int NO_POINT = -1000;  // 标记格子中是否有点，如果没有点高度设置成-1000,
                               // 是一个肯定没有的值
  ...
  for (size_t pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++) {
    ...
    pt.z = _scan_down.points[pt_idx].z + LIDAR_HEIGHT; // naive adding is ok
    // (all points should be > 0).

    // // For G1 ScanContext, we use the following
    // pt.x = _scan_down.points[pt_idx].x;
    // pt.y = -_scan_down.points[pt_idx].y;
...
/**
 * @brief
 * 输入构造的SC矩阵，计算ring-key。其实就是对于矩阵的每一行（对应每一个环），
...
/**
 * @brief
 * 输入构造的SC矩阵，计算sector-key。其实就是对于矩阵的每一列（对应每一个扇区），
...
/**
 * @brief
 * 输入一帧点云，生成ScanContext，并计算ring-key和sector-key，然后存到数据库中
 *
 * @param[in] _scan_down
 */
void SCManager::makeAndSaveScancontextAndKeys(
  ...
  Eigen::MatrixXd sectorkey =
      makeSectorkeyFromScancontext(sc);  // sector-key旋转变化
...
std::pair<int, float> SCManager::detectLoopClosureIDBetweenSession(
    std::vector<float> &_curr_key, Eigen::MatrixXd &_curr_desc) {
  ...
  polarcontext_tree_batch_->index->findNeighbors(
      knnsearch_result, &curr_key[0] /* query */,
...
std::pair<int, float> SCManager::detectClosestKeyframeID(
      ...
      0)  // to save computation cost
  {
    ...
    // KDtreeVectorOfVectorsAdaptor) t_tree_construction.toc("Tree
    // construction");
  }
  ...
  std::vector<size_t> candidate_indexes(
      NUM_CANDIDATES_FROM_TREE);  // 10个最相似候选帧的索引
  std::vector<float> out_dists_sqr(
      NUM_CANDIDATES_FROM_TREE);  // 10个最相似候选帧的距离
  ...
  for (int candidate_iter_idx = 0;
    ...
    if (candidate_dist < min_dist) {
      ...
      nn_idx =
          candidate_indexes[candidate_iter_idx];  // 找到最匹配的关键帧的索引
    }
  }
  // t_calc_dist.toc("Distance calc");
  ...
  if (min_dist < SC_DIST_THRES) {
    ...
    // polarcontexts_.size() - 1 << " and " << nn_idx << "." << endl; cout <<
    // "[Loop found] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." <<
    // endl;
  } else {
    // std::cout.precision(3);
    // cout << "[Not loop] Nearest distance: " << min_dist << " btn " <<
    // polarcontexts_.size() - 1 << " and " << nn_idx << "." << endl; cout <<
    // "[Not loop] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." <<
    // endl;
  }
...
std::pair<int, float> SCManager::detectClosestKeyframeID(
  ...
  for (int candidate_iter_idx = 0;
    ...
    if (candidate_dist < min_dist &&
        std::find(invalid_id.begin(), invalid_id.end(),
                  static_cast<int>(candidate_indexes[candidate_iter_idx])) ==
            invalid_id.end()) {
      min_dist = candidate_dist;
      nn_align = candidate_align;
...
/**
 * @brief
 * 检测闭环对，就是检测数据库中最新的那个点云帧和历史上所有帧之间的回环关系
 *
...
void SCManager::saveCurrentSCD(const std::string &save_path, int num_digits,
                               const std::string &delimiter) {
  const auto &curr_scd = polarcontexts_.back();
  std::ostringstream out;
  out << std::internal << std::setfill('0') << std::setw(num_digits)
      << polarcontexts_.size() - 1;
...
void SCManager::loadPriorSCD(const std::string &path, int num_digits,
                             int num_keyframe) {
  for (auto i = 0; i < num_keyframe; ++i) {
    ...
    if (file.is_open()) {
      for (int i = 0; i < PC_NUM_RING; ++i)
        for (int j = 0; j < PC_NUM_SECTOR; ++j) file >> curr_scd(i, j);
...
std::pair<int, float> SCManager::relocalize(
    pcl::PointCloud<PointTypeXYZI> &scan_down) {
