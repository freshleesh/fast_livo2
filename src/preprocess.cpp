...
void Preprocess::mid360_handler(
  ...
  // timestamp is absolute nanoseconds; compute relative offset in ms from first point
  double t0_ns = pl_orig.points[0].timestamp;
  ...
  for (int i = 0; i < plsize; i++) {
    ...
    added_pt.normal_y = 0;
    added_pt.normal_z = 0;
    // curvature is used as relative time offset in ms
    added_pt.curvature = (pl_orig.points[i].timestamp - t0_ns) / 1e6;
